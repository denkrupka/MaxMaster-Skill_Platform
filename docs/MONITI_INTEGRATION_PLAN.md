...
3. [МОДУЛЬ A: Настройки компании (рабочее время)](#модуль-a-настройки-компании--конфигурация-рабочего-времени)
4. [МОДУЛЬ B: Отделы с иерархией и геозонами](#модуль-b-отделы-с-иерархией-и-геозонами)
5. [МОДУЛЬ C: Учёт рабочего времени (Time & Attendance)](#модуль-c-учёт-рабочего-времени-time--attendance)
6. [МОДУЛЬ D: Дашборд "Команда сейчас"](#модуль-d-дашборд-команда-сейчас-team-now)
7. [МОДУЛЬ E: Отпуска и отсутствия (Time Off)](#модуль-e-отпуска-и-отсутствия-time-off)
8. [МОДУЛЬ F: Рабочие графики (Work Schedule)](#модуль-f-рабочие-графики-work-schedule)
9. [МОДУЛЬ G: Задачи и проекты (Tasks & Projects)](#модуль-g-задачи-и-проекты-tasks--projects)
10. [МОДУЛЬ H: Отчёты и Payroll](#модуль-h-расширенные-отчёты-и-payroll)
11. [МОДУЛЬ I: Праздничный календарь](#модуль-i-праздничный-календарь)
12. [МОДУЛЬ J: Центр уведомлений (Notification Hub)](#модуль-j-центр-уведомлений-notification-hub)
...
| Управление отделами с геозонами | ✅ полный | ✅ чтение | ❌ | ❌ | ❌ |
| Учёт времени — просмотр всех | ✅ | ✅ свой отдел | ✅ свой отдел | ✅ своя бригада | ❌ |
| Учёт времени — отметка (clock-in/out) | ✅ | ✅ | ✅ | ✅ | ✅ |
| Учёт времени — ручная коррекция | ✅ | ✅ | ❌ | ❌ | ❌ |
| Дашборд "Команда сейчас" | ✅ все | ✅ свой отдел | ✅ свой отдел | ✅ своя бригада | ❌ |
...
| Праздничный календарь | ✅ настройка | ✅ чтение | ✅ чтение | ✅ чтение | ✅ чтение |
| Заявки на изменение рабочего дня | ✅ одобрение | ✅ одобрение | ❌ | ❌ | ✅ подача |
...
| 2 | Иерархия отделов с геозонами | ❌ Нет | Новый модуль |
| 3 | Учёт рабочего времени (clock-in/out) | ❌ Нет | Новый модуль |
| 4 | Дашборд "Команда сейчас" (real-time) | ❌ Нет | Новый модуль |
...
| 10 | Праздничный календарь | ❌ Нет | Новый модуль |
...
## 3. ПОРЯДОК ВНЕДРЕНИЯ МОДУЛЕЙ

```
МОДУЛЬ A: Настройки компании (рабочее время)     ← Фундамент
    ↓
МОДУЛЬ B: Отделы с иерархией и геозонами          ← Фундамент
    ↓
МОДУЛЬ C: Учёт рабочего времени (Time&Attendance) ← Ядро
    ↓
МОДУЛЬ D: Дашборд "Команда сейчас" (Team Now)     ← Зависит от C
    ↓
МОДУЛЬ E: Отпуска и отсутствия (Time Off)          ← Независимый
    ↓
МОДУЛЬ F: Рабочие графики (Work Schedule)          ← Зависит от C, E
    ↓
МОДУЛЬ G: Задачи и проекты (Tasks & Projects)      ← Независимый
    ↓
МОДУЛЬ H: Расширенные отчёты и Payroll             ← Зависит от C, E, F
    ↓
МОДУЛЬ I: Праздничный календарь                     ← Независимый
    ↓
МОДУЛЬ J: Центр уведомлений (Notification Hub)     ← Расширение
```

---

## МОДУЛЬ A: Настройки компании — Конфигурация рабочего времени
...
-- 3. RLS политики — только COMPANY_ADMIN своей компании может редактировать
...
```typescript
// === МОДУЛЬ A: Настройки рабочего времени ===
...
- [ ] Только COMPANY_ADMIN видит и может редактировать настройки
- [ ] Дефолтные значения корректно применяются для новых компаний

---

## МОДУЛЬ B: Отделы с иерархией и геозонами

### B.1 Описание модуля

Создать полноценную систему отделов с поддержкой иерархии (вложенности), привязкой сотрудников к отделам, и опциональными геозонами (GPS-координаты + радиус). В Moniti отделы — ключевая структурная единица: сотрудники назначены в отделы, менеджеры управляют конкретными отделами, отделы имеют адреса и геозоны для строительных объектов.

### B.2 Зависимости

- Таблица `companies` (есть)
- Таблица `users` (есть)
- Модуль A (настройки компании) — рекомендуется, но не обязательно
...
-- 2. Таблица связи "Сотрудник ↔ Отдел" (many-to-many)
CREATE TABLE department_members (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  department_id UUID NOT NULL REFERENCES departments(id) ON DELETE CASCADE,
  user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  company_id UUID NOT NULL REFERENCES companies(id) ON DELETE CASCADE,
  role TEXT DEFAULT 'member',          -- 'member' | 'manager'  (manager = менеджер этого отдела)
...
-- departments: пользователи видят только отделы своей компании
...
#### B.5.1 Страница списка отделов

**Путь:** `/company/departments` (новая страница)
**Доступ:** COMPANY_ADMIN (полный), HR (чтение + назначение сотрудников)

**Элементы страницы:**
- **Заголовок:** "Отделы" с кнопкой "+ Добавить отдел"
- **Дерево отделов** — иерархический список:
  - Каждый узел: название отдела, кол-во сотрудников, иконка геозоны (если есть координаты)
  - Раскрытие/сворачивание дочерних отделов
  - Кнопки действий: Редактировать, Архивировать, Удалить (только для COMPANY_ADMIN)
  - Кнопка "Добавить подотдел" на каждом узле
- **Фильтры:** Поиск по имени, переключатель "Показать архивные"
- **Альтернативный вид:** Таблица (переключение дерево/таблица)

#### B.5.2 Модальное окно создания/редактирования отдела

**Поля формы:**
- `name` — Название отдела (обязательное, text input)
- `parent_id` — Родительский отдел (выпадающий список существующих отделов, опционально)
...
#### B.5.3 Страница деталей отдела

**Путь:** `/company/departments/:id`
**Вкладки:**
1. **Информация** — название, адрес, геозона, родительский отдел, дочерние отделы
2. **Сотрудники** — список назначенных сотрудников с ролью (member/manager)
   ...
   - Кнопка "Удалить из отдела" для каждого сотрудника
   - Переключатель роли member/manager для каждого сотрудника

### B.6 Навигация

Добавить пункт в сайдбар для ролей COMPANY_ADMIN и HR:
...
- [ ] Страница `/company/departments` с деревом отделов
- [ ] CRUD операции для отделов работают
- [ ] Назначение/удаление сотрудников из отделов работает
- [ ] Иерархия (parent_id) отображается корректно
- [ ] Label автоматически генерируется триггером
- [ ] Пункт "Отделы" в сайдбаре для COMPANY_ADMIN и HR
- [ ] RLS изолирует данные по company_id

---

## МОДУЛЬ C: Учёт рабочего времени (Time & Attendance)
...
- **Модуль A** (настройки рабочего времени компании) — для округления, рабочих часов, макс. времени
- **Модуль B** (отделы) — опционально, для привязки отметки к отделу
- Таблица `users` (есть)
- Таблица `companies` (есть)
...
CREATE TABLE worker_days (
  ...
  -- 'time_off'   = отпуск/отсутствие (из модуля E)
...
-- (напр. ушёл на обед и вернулся = 2 entries, или работал в двух отделах).
CREATE TABLE worker_day_entries (
  ...
  -- Привязка к отделу / должности (опционально)
...
```typescript
// === МОДУЛЬ C: Учёт рабочего времени ===
...
- Опциональный выбор отдела (dropdown, если сотрудник в нескольких)
...
**Доступ:** COMPANY_ADMIN (все), HR (все в компании), COORDINATOR (свой отдел), BRIGADIR (своя бригада)
...
- Таблица всех сотрудников:
  - Имя | Отдел | Статус | Приход | Уход | Работа | Перерыв | Переработка | Действия
- Фильтры: по отделу, по статусу (present/absent/late/etc.), поиск по имени
...
#### C.5.3 Модальное окно "Редактирование рабочего дня" (для COMPANY_ADMIN, HR)
...
#### C.5.4 Модальное окно "Заявка на изменение" (для EMPLOYEE)
...
**Дополнительно для COMPANY_ADMIN, HR:**
...
*COORDINATOR и BRIGADIR видят только сотрудников своего отдела/бригады (фильтрация через department_members и assigned_brigadir_id)
...
## МОДУЛЬ D: Дашборд "Команда сейчас" (Team Now)

### D.1 Описание модуля

Дашборд реального времени, показывающий текущий статус всех сотрудников компании: кто работает, кто на перерыве, кто ещё не пришёл. В Moniti это ключевой экран для менеджеров. Данные берутся из таблицы `worker_states` (модуль C). Обновление через Supabase Realtime (подписка на изменения таблицы).

### D.2 Зависимости

- **Модуль C** (Time & Attendance) — таблица `worker_states` обязательна
- **Модуль B** (Отделы) — опционально, для фильтрации по отделам

### D.3 База данных

Новых таблиц не требуется. Используется `worker_states` из модуля C.

### D.4 UI — Страница "Команда сейчас"

**Путь:** `/company/team-now` (новая страница)
**Доступ:** COMPANY_ADMIN, HR (все), COORDINATOR (свой отдел), BRIGADIR (своя бригада)
...
- Таблица с автообновлением:
  - Аватар | Имя Фамилия | Отдел | Статус (цветной бейдж) | Текущая активность с | Начало работы | Общее время сегодня
...
- Фильтры:
  ...
  - По отделу (dropdown, если модуль B внедрён)
  - Поиск по имени
...
| Действие | COMPANY_ADMIN | HR | COORDINATOR | BRIGADIR | EMPLOYEE |
|---|---|---|---|---|---|
| Просмотр дашборда | ✅ все | ✅ все | ✅ свой отдел | ✅ своя бригада | ❌ |
...
- [ ] Фильтрация по статусу, отделу, поиск по имени
- [ ] Цветовое кодирование статусов
- [ ] Пункт в сайдбаре для управленческих ролей
- [ ] COORDINATOR видит только свой отдел, BRIGADIR — свою бригаду

---

## МОДУЛЬ E: Отпуска и отсутствия (Time Off)
...
- Таблица `users` (есть)
- Таблица `companies` (есть)
- Модуль C (опционально) — для интеграции: день с одобренным отпуском помечается статусом `time_off` в `worker_days`
...
#### E.5.2 Страница "Управление отпусками" (для COMPANY_ADMIN, HR)

**Путь:** `/company/time-off` (новая страница)
**Доступ:** COMPANY_ADMIN, HR
...
const approveRequest = async (requestId: string) => {
  ...
  // 3. (Если модуль C) Создать/обновить worker_days со статусом 'time_off'
...
## МОДУЛЬ F: Рабочие графики (Work Schedule)
...
- Таблица `users` (есть)
- Модуль B (Отделы) — опционально
- Модуль E (Time Off) — для отображения одобренных отпусков в графике
...
- Отпуска отображаются отдельным цветом (если модуль E)
...
- Фильтры: по отделу, по сотруднику
...
## МОДУЛЬ G: Задачи и проекты (Tasks & Projects)
...
- Таблица `users` (есть)
- Таблица `companies` (есть)
- Модуль C (опционально) — для привязки залогированного времени к задачам
...
#### G.5.3 "Задачи" (для COMPANY_ADMIN, HR, COORDINATOR)
...
#### G.5.4 "Клиенты" (для COMPANY_ADMIN, HR)
...
## МОДУЛЬ H: Расширенные отчёты и Payroll

### H.1 Описание модуля

Расширение существующей системы отчётов. Генерация табелей (timesheets) на основе данных из модулей C, E, F. Отчёты по времени и зарплате с учётом ставок, переработок, ночных смен.

### H.2 Зависимости

- Модуль C (Time & Attendance) — основные данные по времени
- Модуль E (Time Off) — опционально, учёт отпусков в табеле
- Модуль F (Work Schedule) — опционально, план vs факт
...
#### H.5.1 Страница "Табели" (для COMPANY_ADMIN, HR)
...
#### H.5.2 Страница "Отчёты" (для COMPANY_ADMIN, HR)

**Путь:** `/company/reports`
- **Отчёт "Время и зарплата":**
  - Фильтры: период, отдел, сотрудники
  - Таблица с агрегацией по сотрудникам
  - Графики (recharts): время по дням, распределение по отделам
  - Экспорт Excel/PDF

- **Отчёт "Посещаемость":**
  - Фильтры: период, отдел
...
## МОДУЛЬ I: Праздничный календарь
...
- [ ] Интеграция с модулями C, F, H (пометка дней как holiday)

---

## МОДУЛЬ J: Центр уведомлений (Notification Hub)

### J.1 Описание модуля

Расширение существующей системы уведомлений. Централизованный хаб с историей всех уведомлений, статусом прочтения, фильтрацией. Новые типы уведомлений для модулей C–I.

### J.2 Зависимости

- Существующая таблица уведомлений (расширяется)
- Все модули (C–I) генерируют уведомления
...
- Модуль C: новая заявка на изменение дня → уведомление admin/hr. Одобрение/отклонение → уведомление сотруднику.
- Модуль E: новая заявка на отпуск → уведомление admin/hr. Одобрение/отклонение → уведомление сотруднику.
- Модуль F: обновление графика → уведомление затронутых сотрудников.
- Модуль G: назначение задачи → уведомление исполнителю. Изменение статуса → уведомление создателю.
- Модуль H: табель сформирован → уведомление сотруднику.
...
- [ ] Генерация уведомлений из модулей C–I
...
| Модуль | Таблицы БД | Новые страницы | Приоритет |
|---|---|---|---|
| A. Настройки рабочего времени | 0 (расширение companies) | 0 (расширение settings) | 1 - Фундамент |
| B. Отделы с иерархией | 2 (departments, department_members) | 2 (список, детали) | 1 - Фундамент |
| C. Учёт рабочего времени | 6 (worker_days, entries, activities, actions, requests, states) | 2 (мой день, посещаемость) | 2 - Ядро |
| D. Дашборд "Команда сейчас" | 0 (использует worker_states) | 1 (team-now) | 2 - Ядро |
| E. Отпуска | 3 (types, limits, requests) | 2 (мои отпуска, управление) | 3 - Расширение |
| F. Рабочие графики | 2 (templates, assignments) | 2 (мой график, планирование) | 3 - Расширение |
| G. Задачи и проекты | 6 (customers, projects, members, tasks, time_logs, attachments) | 4 (задачи, проекты, клиенты, детали) | 3 - Расширение |
| H. Отчёты и Payroll | 2 (timesheets, saved_reports) | 2 (табели, отчёты) | 4 - Продвинутый |
| I. Праздничный календарь | 1 (holiday_days) | 0 (вкладка в settings) | 4 - Продвинутый |
| J. Центр уведомлений | 1 (notifications) | 1 (уведомления) | 4 - Продвинутый |
